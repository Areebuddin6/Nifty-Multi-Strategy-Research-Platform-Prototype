    ...
    <div className="grid grid-cols-2 md:grid-cols-4 lg:grid-cols-6 gap-3 mb-6">
      ...
      <div className="bg-slate-900 border border-slate-800 rounded-xl p-3.5 flex flex-col justify-between transition-colors shadow-sm">
        ...
        <div className={`text-xs font-mono flex items-center space-x-1 mt-1 font-semibold ${isProfit ? 'text-emerald-500' : 'text-rose-500'}`}>
          ...
          <span>{isProfit ? '+' : ''}{stats.totalReturnPct.toFixed(1)}% net</span>
        </div>
      </div>
      ...
      <div className="bg-slate-900 border border-slate-800 rounded-xl p-3.5 flex flex-col justify-between transition-colors shadow-sm">
        ...
        <div className="text-xs font-mono text-slate-400 mt-1 flex items-center space-x-1">
          <span>Sortino:</span>
          <span className="text-slate-200 font-semibold">{stats.sortinoRatio.toFixed(2)}</span>
      ...
      <div className="bg-slate-900 border border-slate-800 rounded-xl p-3.5 flex flex-col justify-between transition-colors shadow-sm">
        ...
        <div className="text-xs font-mono text-slate-400 mt-1 flex items-center space-x-1">
          <span>Calmar:</span>
          <span className="text-slate-200 font-semibold">{stats.calmarRatio.toFixed(2)}</span>
      ...
      <div className="bg-slate-900 border border-slate-800 rounded-xl p-3.5 flex flex-col justify-between transition-colors shadow-sm">
        ...
        <div className="text-xs font-mono text-slate-400 mt-1 flex items-center space-x-1">
          <span>PF:</span>
          <span className="text-slate-200 font-semibold">{stats.profitFactor.toFixed(2)}</span>
          <MetricHelpButton metricId="profit_factor" onOpenHelp={handleHelp} size="xs" />
          <span className="text-slate-500 text-[10px]">({stats.totalTrades} tr)</span>
        </div>
      </div>
